...
makedirs(void)
{
	...
	rootuser = ialloc(T_DIR);

	bzero(&de, sizeof(de));
	de.inum = xshort(rootuser);
	strcpy(de.name, ".");
	iappend(rootuser, &de, sizeof(de));
...
main(int argc, char *argv[])
{
	...
	for(i = 2; i < argc; i++){
		...
		if(strncmp(argv[i],"motd",4)==0 || strncmp(argv[i],"issue",5)==0 || 
			strncmp(argv[i],"passwd",6)==0 || strncmp(argv[i],"group",5)==0)
			dirino = etcino;
		else
			dirino = rootuser;

		if(strncmp(shortname,"_passwd",7)==0)
			setuidnum=02000;
		else
			setuidnum=0;
		if(strncmp(argv[i], "user/", 5) == 0){
			setuidnum = setuidnum | 0111;
		}
